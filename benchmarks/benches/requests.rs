...
main!(
    config = LibraryBenchmarkConfig::default()
        ...
        // want to trace.
        // num-callers: We only want to measure allocs / instructions made
        // by benchmark function, not setup or teardown. Allocs / instructions
        // are attributed to benchmark (or setup/teardown) using call stack.
        // `num-callers` controls the maximum depth of analyzed call stack.
        // If it is low, then the benchmark function won't appear in call stack,
        // so we will miss the allocs / instructions, making the results fake.
        // The tradeoff is that bigger values make benchmarks slower (without
        // affecting their results ofc). 500 is the maximum value, and the speed
        // is acceptable to me, which is why I chose it.
