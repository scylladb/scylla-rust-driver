...
The driver keeps an up-to-date view of the cluster topology and of the cluster schema. Most of it is refreshed in reaction to the server events announcing a change, so no polling is involved. Schema changes are the exception: the keyspaces that `SCHEMA_CHANGE` events name are accumulated and re-read by a periodic refresh, which batches the burst of events that a single DDL statement produces.
...
Only the affected keyspaces are re-read, not the whole schema. If your cluster's events cannot be relied upon, `periodic_metadata_fetch_mode(PeriodicFetchMode::FullMetadata)` makes each refresh re-read the whole metadata instead - the behaviour of driver versions that did not handle `SCHEMA_CHANGE` events.
