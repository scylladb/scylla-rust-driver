...
impl<'sd> TypeCheckUnorderedGenerator<'sd> {
    ...
    // Generates the type_check method.
    fn generate(&self) -> syn::ImplItemFn {
        ...
        let rust_field_names = rust_fields
            .iter()
            .filter(|f| !f.skip)
            ...
            fn type_check(
                ...
                for (cql_field_name, cql_field_typ) in cql_fields {
                    ...
                    match cql_field_name.as_str() {
                        #(#rust_field_names => #type_check_blocks,)*
                        _unknown => {
        ...
        parse_quote! {
            fn deserialize(
                ...
                for item in cql_field_iter {
                    ...
                    match cql_field_name.as_str() {
                        #(#rust_field_names => #deserialize_blocks,)*
                        unknown => panic!(
                            "unmatched field in UDT: {} - type check should have prevented this!",
                            cql_field_name
                        )
                    }
                }
