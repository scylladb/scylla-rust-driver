...
impl<'sd> TypeCheckAssumeOrderGenerator<'sd> {
    ...
    fn generate(&self) -> syn::ImplItemMethod {
        ...
        let required_field_count = required_fields.len();
        let field_count_lit =
        ...
        let name_verifications: Vec<_> = required_fields
            .iter()
            .enumerate()
            .map(|(id, field)| self.generate_name_verification(id, field, &parse_quote!(name)))
            .collect();
