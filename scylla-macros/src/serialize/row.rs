...
impl Generator for ColumnSortingGenerator<'_> {
    fn generate_serialize(&self) -> syn::TraitItemFn {
        ...
        let serialize_field_block: syn::Block = if self.ctx.fields.is_empty() {
            parse_quote! {{
                ::std::result::Result::Ok(#crate_path::FieldRowSerializationStatus::NotUsed)
            }}
        } else {
            parse_quote! {{
                match spec.name() {
                    #(#unflattened_columns => {
                        ...
                        <#unflattened_tys as #crate_path::SerializeValue>::serialize(&self.#unflattened_fields, spec.typ(), sub_writer)?;
                        self.missing.remove(#unflattened_columns);
                    })*
                    _ => {
                        #({
                            match self.#flattened_fields.serialize_field(spec, writer)? {
                                #crate_path::FieldRowSerializationStatus::Done => {
                                    self.missing.remove(stringify!(#flattened_fields));
                                    if self.missing.is_empty() {
                                        return ::std::result::Result::Ok(#crate_path::FieldRowSerializationStatus::Done);
                                    } else {
                                        return ::std::result::Result::Ok(#crate_path::FieldRowSerializationStatus::NotDone);
                                    }
                                }
                                #crate_path::FieldRowSerializationStatus::NotDone => {
                                    return ::std::result::Result::Ok(#crate_path::FieldRowSerializationStatus::NotDone)
                                }
                                #crate_path::FieldRowSerializationStatus::NotUsed => {}
                            };
                        })*

                        return ::std::result::Result::Ok(#crate_path::FieldRowSerializationStatus::NotUsed);
                    }
                }

                if self.missing.is_empty() {
                    ::std::result::Result::Ok(#crate_path::FieldRowSerializationStatus::Done)
                } else {
                    ::std::result::Result::Ok(#crate_path::FieldRowSerializationStatus::NotDone)
                }
            }}
        };
        ...
        let partial_serialize: syn::ItemImpl = parse_quote! {
            impl #partial_impl_generics #crate_path::PartialSerializeRowByName for #partial_struct_name #partial_ty_generics #partial_where_clause {
                ...
                ) -> ::std::result::Result<#crate_path::FieldRowSerializationStatus, #crate_path::SerializationError> {
                ...
                fn check_missing(self) -> ::std::result::Result<(), #crate_path::SerializationError> {
                    ...
                    match missing {
                        ...
                        _ => ::std::result::Result::Err(#crate_path::SerializationError::new(#crate_path::BuiltinRowTypeCheckError {
                            rust_name: ::std::any::type_name::<#struct_name #ty_generics>(),
                            kind: #crate_path::BuiltinRowTypeCheckErrorKind::ValueMissingForColumn {
        ...
        parse_quote! {
            fn serialize<'b>(
                ...
                <Self as #crate_path::SerializeRowByName>::serialize_by_name(self, ctx, writer)
            }
        }
    }
