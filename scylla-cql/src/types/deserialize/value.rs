...
/// An iterator over either a CQL vector
    ...
    T: DeserializeValue<'frame>,
{
    fn type_check(typ: &ColumnType) -> Result<(), TypeCheckError> {
        match typ {
            ColumnType::Vector(el_t, _) => {
                if !matches!(el_t.as_ref(), ColumnType::Float) {
                    return Err(mk_typck_err::<Self>(
                        typ,
                        BuiltinTypeCheckErrorKind::VectorError(
                            VectorTypeCheckErrorKind::UnsupportedElementType,
                        ),
                    ));
                }
                <T as DeserializeValue<'frame>>::type_check(el_t).map_err(|err| {
            ...
            _ => Err(mk_typck_err::<Self>(
                typ,
                BuiltinTypeCheckErrorKind::VectorError(VectorTypeCheckErrorKind::NotVector),
            )),
        }
    }
    ...
    fn deserialize(
        ...
        let ColumnType::Vector(elem_typ, dimensions) = typ else {
            unreachable!("Typecheck should have prevented this scenario!")
        };
        // TODO: This should be better associated with ColumnType
        let size_of_cql_float = 4;
        Ok(Self::new(
            typ,
            elem_typ,
            *dimensions as usize,
...
impl<'frame, T> Iterator for VectorIterator<'frame, T>
where
    T: DeserializeValue<'frame>,
{
    type Item = Result<T, DeserializationError>;
    ...
    T: DeserializeValue<'frame>,
{
    fn type_check(typ: &ColumnType) -> Result<(), TypeCheckError> {
        ...
        match typ {
            ...
            ColumnType::Vector(_, _) => VectorIterator::<'frame, T>::type_check(typ)
                .map_err(typck_error_replace_rust_name::<Self>),
            _ => Err(mk_typck_err::<Self>(
                typ,
                BuiltinTypeCheckErrorKind::SetOrListError(
...
impl Display for SetOrListTypeCheckErrorKind {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            SetOrListTypeCheckErrorKind::NotSetOrList => {
                f.write_str("the CQL type the Rust type was attempted to be type checked against was neither a set, list nor a vector")
            }
            SetOrListTypeCheckErrorKind::NotSet => {
...
impl Display for VectorTypeCheckErrorKind {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            VectorTypeCheckErrorKind::UnsupportedElementType => {
                f.write_str("only float elements are supported in CQL vectors")
            }
