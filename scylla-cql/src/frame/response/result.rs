...
fn deser_table_spec(
    buf: &mut &[u8],
    known_spec: Option<TableSpec<'static>>,
) -> StdResult<TableSpec<'static>, ParseError> {
    let ks_name = types::read_string(buf)?;
    let table_name = types::read_string(buf)?;

    match known_spec {
        None => Ok(TableSpec::owned(ks_name.to_owned(), table_name.to_owned())),
        Some(table_spec) => {
...
fn deser_col_specs(
    ...
    col_count: usize,
) -> StdResult<(Option<TableSpec<'static>>, Vec<ColumnSpec>), ParseError> {
