...
use crate::frame::response::new_type_parser;
...
impl ColumnType<'_> {
    ...
    /// Returns size of a type in bytes, if it's fixed-size.
    ...
    Ok(match id {
        0x0000 => {
            ...
            let result = match type_cow {
                Cow::Borrowed(type_name) => new_type_parser::TypeParser::parse(type_name),
                Cow::Owned(_) => type_parser::TypeParser::parse(type_cow.clone()),
            };
        ...
        pub(crate) fn serialize(&self, buf: &mut impl BufMut) -> StdResult<(), TryFromIntError> {
            ...
            match self {
                ...
                ColumnType::Vector(_, _) => {
                    unimplemented!("Vector metadata serialization is not implemented");
                }
