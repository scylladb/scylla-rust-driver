...
struct UDTParameters<'result> {
    ...
    name: Cow<'result, str>,
    fields: Vec<(Cow<'result, str>, ColumnType<'result>)>,
}
...
impl<'result> CustomTypeParser<'result> {
    fn new(parsed_string: Cow<'result, str>) -> CustomTypeParser<'result> {
        CustomTypeParser {
            pos: 0,
            parsed_string,
        }
    ...
    pub(crate) fn parse(
        ...
        let mut parser = CustomTypeParser::new(str);
        parser.do_parse()
    }
    ...
    fn read_next_identifier(&mut self) -> Cow<'result, str> {
        let start = self.pos;
        loop {
            // We read until we find a non-identifier character
            match self.char_at_pos() {
                Some(c) if CustomTypeParser::is_identifier_char(c) => {
                    self.pos += 1;
                }
                _ => {
                    break;
    ...
    fn skip_blank(&mut self) -> usize {
        loop {
            match self.char_at_pos() {
                Some(c) if CustomTypeParser::is_blank(c) => {
                    self.pos += 1;
                }
                _ => {
                    break;
    ...
    fn do_parse(&mut self) -> Result<ColumnType<'result>, CustomTypeParseError> {
        ...
        // Custom type can be prefixed with a hex string, we discard it
        if let Some(':') = self.char_at_pos() {
            self.pos += 1;
            let _ =
        ...
        match self.char_at_pos() {
            ...
            _ => CustomTypeParser::get_simple_abstract_type(name),
        }
    }
}
