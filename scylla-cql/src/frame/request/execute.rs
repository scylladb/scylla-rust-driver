...
use crate::frame::response::result::cow_bytes::{self, CowBytes};
use bytes::Bytes;
use thiserror::Error;
...
#[deprecated(since = "1.4.0", note = "Use ExecuteWithResultMetadataId instead")]
...
pub struct ExecuteV2<'a> {
    /// ID of the prepared statement to execute.
    pub id: cow_bytes::CowBytes<'a>,

    /// ID of the result metadata stored locally.
    pub result_metadata_id: Option<cow_bytes::CowBytes<'a>>,
...
impl DeserializableRequest for ExecuteV2<'_> {
    fn deserialize(buf: &mut &[u8]) -> Result<Self, RequestDeserializationError> {
