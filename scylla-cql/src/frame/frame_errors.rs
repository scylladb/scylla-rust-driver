...
pub enum FrameDeserializationError {
    ...
    #[error("Failed to read a frame header: {0}")]
    HeaderIoError(Arc<std::io::Error>),
    #[error("Failed to read a chunk of response body. Expected {0} more bytes, error: {1}")]
    BodyChunkIoError(usize, Arc<std::io::Error>),
...
impl FrameDeserializationError {
    pub fn new_header_io_error(err: std::io::Error) -> Self {
        Self::HeaderIoError(Arc::new(err))
    }

    pub fn new_body_chunk_io_error(expected_bytes: usize, err: std::io::Error) -> Self {
        Self::BodyChunkIoError(expected_bytes, Arc::new(err))
    }
}
