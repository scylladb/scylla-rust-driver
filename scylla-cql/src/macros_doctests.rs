// This file contains doctests for macros (`SerializeRow`, `SerializeValue`,
// `DeserializeRow`, `DeserializeValue`). All macro-related doctests should be placed here,
// rather than alongside other serialization/deserialization tests. The reason for this
// separation is that doctests cannot be placed inside `#[cfg(test)]` modules - they are
// only supported at the crate level or in regular (non-test) modules.
