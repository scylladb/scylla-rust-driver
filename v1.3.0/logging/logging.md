...
The driver uses the [tracing](https://github.com/tokio-rs/tracing) crate for all logs.<br />
\\\\
There are two ways to view the logs:

- Create a `tracing` subscriber to which all logs will be written (recommended).
- Enable `log` feature on `tracing` crate and use some logger from `log` ecosystem. <br />
  \\\\
