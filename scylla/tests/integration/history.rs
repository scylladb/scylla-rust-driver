...
use scylla::transport::errors::{
    TimeoutableRequestError, UserRequestAttemptError, UserRequestError,
};
...
fn set_one_db_error_message(mut history: StructuredHistory) -> StructuredHistory {
    let set_msg_attempt = |err: &mut UserRequestAttemptError| {
        if let UserRequestAttemptError::DbError(_, msg) = err {
            *msg = "Error message from database".to_string();
        }
    };
    let set_msg_request_error = |err: &mut TimeoutableRequestError| {
        if let TimeoutableRequestError::RequestFailure(UserRequestError::LastAttemptError(
            UserRequestAttemptError::DbError(_, msg),
        )) = err
        {
            *msg = "Error message from database".to_string();
        }
