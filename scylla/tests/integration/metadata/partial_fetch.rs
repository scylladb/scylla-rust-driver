...
/// `.scylla_keyspaces` (tablet information) and `.scylla_aggregates` (reduce
/// functions). A full fetch would add `system.peers` and `system.local`.
/// The last 3 tables are not present on Cassandra, so they won't be queried.
const SCHEMA_FETCH_REQUESTS: usize = if cfg!(cassandra_tests) { 8 } else { 11 };
