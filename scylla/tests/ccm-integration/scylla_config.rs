...
impl NodeConfig {
    ...
    pub(crate) fn output_mut(self: &mut NodeConfig) -> Option<&mut NodeConfig> {
        match self {
            NodeConfig::List(list) => list.last_mut(),
            NodeConfig::Map(map) => map.values_mut().last(),
            _ => None,
        }
    }
    ...
    /// towards completion.
    pub(crate) fn take_output(self: &mut NodeConfig) -> Option<NodeConfig> {
        match self {
            NodeConfig::List(list) => list.pop(),
            NodeConfig::Map(map) => map.values_mut().next().map(|value| value.clone()),
            _ => None,
        }
    }
}
...
mod tests {
    ...
    fn test_to_flat_string_nested_map() {
        ...
        outer_map.insert("outer_key2".to_string(), NodeConfig::Float(3.14));
