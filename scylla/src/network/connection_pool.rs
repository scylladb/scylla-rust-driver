            ...
            tokio::select! {
                ...
                _ = self.refill_now_notify.notified() => {
                    ...
                    if self.need_filling() {
                        debug!(
                            "[{}] Pool needs filling, scheduling immediate refill",
                            self.endpoint_description()
                        );
                        // Schedule the refill immediately, without waiting for the backoff delay.
                        next_refill_time = tokio::time::Instant::now();
