...
pub enum Strategy {
    ...
    /// - replicas in each datacenter: This configuration tolerates the failure of a single node
    ///   per replication group and still allows local reads at a consistency level of ONE.
    /// - replicas in each datacenter: This configuration tolerates either the failure of one node
    ///   per replication group at a strong consistency level of LOCAL_QUORUM or multiple node failures
    ///   per datacenter using consistency level ONE.
    ///
    /// Asymmetrical replication groupings are also possible. For example, you can have three replicas
    /// in one datacenter to serve real-time application requests and use a single replica elsewhere
