...
use crate::cluster::node::{
    resolve_contact_points, InternalKnownNode, NodeAddr, ResolvedContactPoint,
};
use crate::deserialize::DeserializeOwnedRow;
use crate::errors::{
    DbError, KeyspaceStrategyError, KeyspacesMetadataError, MetadataError, MetadataFetchError,
    MetadataFetchErrorKind, NewSessionError, PeersMetadataError, RequestAttemptError, RequestError,
    ...
    async fn query_keyspaces(
        ...
        let (mut all_tables, mut all_views, mut all_user_defined_types) = if fetch_schema {
            ...
            let mut tables_schema = self
                .clone()
                .query_tables_schema(keyspaces_to_fetch, &udts)
                .await?;
            (
                ...
                self.clone()
                    .query_tables(keyspaces_to_fetch, &mut tables_schema)
                    .await?,
