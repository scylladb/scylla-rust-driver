...
impl ControlConnection {
    ...
    /// Performs a partial fetch of the schema metadata: re-reads the schema of
    /// `to_fetch` alone, and reports `dropped` as absent without reading
    /// anything for it.
    ///
    /// The keyspaces come from SCHEMA_CHANGE events - `dropped` being those
    /// whose last event dropped the keyspace itself, which therefore only needs
    /// removing from the metadata.
    ///
    /// Both sets are filtered by this control connection's configuration: a
    /// keyspace the session was told not to fetch must not enter the metadata
    /// through an event either.
    ///
    ...
    pub(super) async fn fetch_schema_update(
        ...
        // Later this wam will be extended with a fetch result.
