...
pub struct Legacy08QueryResult {
...
impl Legacy08QueryResult {
    /// Returns the number of received rows.\
    /// Fails when the query isn't of a type that could return rows, same as [`rows()`](Legacy08QueryResult::rows).
    ...
    /// If `Legacy08QueryResult.rows` is `None`, which means that this query is not supposed to return rows (e.g `INSERT`), returns an error.\
    ...
    /// Opposite of [`rows()`](Legacy08QueryResult::rows).
    ...
    /// Returns rows when `Legacy08QueryResult.rows` is `Some`, otherwise an empty Vec.\
    ...
    /// When `Legacy08QueryResult.rows` is `None`, returns 0 rows.\
...
/// [`Legacy08QueryResult::rows()`](Legacy08QueryResult::rows) or a similar function called on a bad Legacy08QueryResult.\
/// Expected `Legacy08QueryResult.rows` to be `Some`, but it was `None`.\
/// `Legacy08QueryResult.rows` is `Some` for queries that can return rows (e.g `SELECT`).\
/// It is `None` for queries that can't return rows (e.g `INSERT`).
#[derive(Debug, Clone, Error, PartialEq, Eq)]
#[error(
    "Legacy08QueryResult::rows() or similar function called on a bad Legacy08QueryResult.
         Expected Legacy08QueryResult.rows to be Some, but it was None.
         Legacy08QueryResult.rows is Some for queries that can return rows (e.g SELECT).
         It is None for queries that can't return rows (e.g INSERT)."
)]
...
/// [`Legacy08QueryResult::result_not_rows()`](Legacy08QueryResult::result_not_rows) called on a bad Legacy08QueryResult.\
/// Expected `Legacy08QueryResult.rows` to be `None`, but it was `Some`.\
/// `Legacy08QueryResult.rows` is `Some` for queries that can return rows (e.g `SELECT`).\
/// It is `None` for queries that can't return rows (e.g `INSERT`).
#[derive(Debug, Clone, Error, PartialEq, Eq)]
#[error(
    "Legacy08QueryResult::result_not_rows() called on a bad Legacy08QueryResult.
         Expected Legacy08QueryResult.rows to be None, but it was Some.
         Legacy08QueryResult.rows is Some for queries that can return rows (e.g SELECT).
         It is None for queries that can't return rows (e.g INSERT)."
)]
...
pub enum FirstRowError {
    /// [`Legacy08QueryResult::first_row()`](Legacy08QueryResult::first_row) called on a bad Legacy08QueryResult.\
    /// Expected `Legacy08QueryResult.rows` to be `Some`, but it was `None`.\
    /// `Legacy08QueryResult.rows` is `Some` for queries that can return rows (e.g `SELECT`).\
    /// It is `None` for queries that can't return rows (e.g `INSERT`).
    #[error(transparent)]
    ...
    /// Rows in `Legacy08QueryResult` are empty
    #[error("Rows in Legacy08QueryResult are empty")]
    RowsEmpty,
}

#[derive(Debug, Clone, Error, PartialEq, Eq)]
pub enum FirstRowTypedError {
    /// [`Legacy08QueryResult::first_row_typed()`](Legacy08QueryResult::first_row_typed) called on a bad Legacy08QueryResult.\
    /// Expected `Legacy08QueryResult.rows` to be `Some`, but it was `None`.\
    /// `Legacy08QueryResult.rows` is `Some` for queries that can return rows (e.g `SELECT`).\
    /// It is `None` for queries that can't return rows (e.g `INSERT`).
    #[error(transparent)]
...
pub enum MaybeFirstRowTypedError {
    /// [`Legacy08QueryResult::maybe_first_row_typed()`](Legacy08QueryResult::maybe_first_row_typed) called on a bad Legacy08QueryResult.\
    /// Expected `Legacy08QueryResult.rows` to be `Some`, but it was `None`.
    /// `Legacy08QueryResult.rows` is `Some` for queries that can return rows (e.g `SELECT`).\
    /// It is `None` for queries that can't return rows (e.g `INSERT`).
    #[error(transparent)]
...
pub enum SingleRowError {
    /// [`Legacy08QueryResult::single_row()`](Legacy08QueryResult::single_row) called on a bad Legacy08QueryResult.\
    /// Expected `Legacy08QueryResult.rows` to be `Some`, but it was `None`.\
    /// `Legacy08QueryResult.rows` is `Some` for queries that can return rows (e.g `SELECT`).\
    /// It is `None` for queries that can't return rows (e.g `INSERT`).
    #[error(transparent)]
...
pub enum SingleRowTypedError {
    /// [`Legacy08QueryResult::single_row_typed()`](Legacy08QueryResult::single_row_typed) called on a bad Legacy08QueryResult.\
    /// Expected `Legacy08QueryResult.rows` to be `Some`, but it was `None`.\
    /// `Legacy08QueryResult.rows` is `Some` for queries that can return rows (e.g `SELECT`).\
    /// It is `None` for queries that can't return rows (e.g `INSERT`).
    #[error(transparent)]
...
mod tests {
    ...
    fn make_not_rows_query_result() -> Legacy08QueryResult {
    ...
    fn make_rows_query_result(rows_num: usize) -> Legacy08QueryResult {
        let mut res = make_not_rows_query_result();
        res.rows = Some(make_rows(rows_num));
        res
    }

    fn make_string_rows_query_result(rows_num: usize) -> Legacy08QueryResult {
        let mut res = make_not_rows_query_result();
