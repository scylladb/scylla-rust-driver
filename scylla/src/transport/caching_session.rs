...
mod tests {
    ...
    async fn new_for_test(should_disable_tablet_support: bool) -> Session {
        ...
        if should_disable_tablet_support && scylla_supports_tablets(&session).await {
    ...
    async fn create_caching_session() -> CachingSession {
        let session = CachingSession::from(new_for_test(false).await, 2);
    ...
    async fn test_custom_hasher() {
        ...
        let _session: CachingSession<std::collections::hash_map::RandomState> =
            CachingSession::from(new_for_test(false).await, 2);
        let _session: CachingSession<CustomBuildHasher> =
            CachingSession::from(new_for_test(false).await, 2);
        let _session: CachingSession<CustomBuildHasher> =
            CachingSession::with_hasher(new_for_test(false).await, 2, Default::default());
    }
