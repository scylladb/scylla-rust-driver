...
use crate::transport::{
    errors::{RetriableRequestError, UserRequestError},
    metrics::Metrics,
};
...
fn can_be_ignored(error: &RetriableRequestError) -> bool {
    ...
    match error {
        // No point to continue speculative execution if plan is empty.
        RetriableRequestError::EmptyPlan => false,

        // Can try on another node.
        RetriableRequestError::ConnectionPoolError { .. } => true,

        RetriableRequestError::RequestFailure(e) => {
            // Do not remove this lint!
            // It's there for a reason - we don't want new variants
            // automatically fall under `_` pattern when they are introduced.
            #[deny(clippy::wildcard_enum_match_arm)]
            match e {
                ...
                UserRequestError::SerializationError(_)
                ...
                | UserRequestError::RepreparedIdMissingInBatch => false,

                // Errors that can be ignored
                UserRequestError::BrokenConnectionError(_)
                | UserRequestError::UnableToAllocStreamId => true,

                // Handle DbErrors
                UserRequestError::DbError(db_error, _) => {
                    // Do not remove this lint!
                    // It's there for a reason - we don't want new variants
                    // automatically fall under `_` pattern when they are introduced.
                    #[deny(clippy::wildcard_enum_match_arm)]
...
) -> Result<ResT, RetriableRequestError>
where
    QueryFut: Future<Output = Result<ResT, RetriableRequestError>>,
{
    ...
    let mut last_error: RetriableRequestError;
    loop {
        futures::select! {
            _ = &mut sleep => {
