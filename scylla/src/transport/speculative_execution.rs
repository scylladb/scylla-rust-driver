...
use crate::transport::{
    errors::{UserRequestAttemptError, UserRequestError},
    metrics::Metrics,
};
...
fn can_be_ignored(error: &UserRequestError) -> bool {
    ...
    match error {
        ...
        UserRequestError::EmptyPlan => false,

        // Can try on another node.
        UserRequestError::ConnectionPoolError { .. } => true,

        UserRequestError::LastAttemptError(e) => {
            // Do not remove this lint!
            // It's there for a reason - we don't want new variants
            // automatically fall under `_` pattern when they are introduced.
            #[deny(clippy::wildcard_enum_match_arm)]
            match e {
                ...
                UserRequestAttemptError::SerializationError(_)
                ...
                | UserRequestAttemptError::UnexpectedResponse(_)
                | UserRequestAttemptError::RepreparedIdChanged { .. }
                | UserRequestAttemptError::RepreparedIdMissingInBatch => false,

                // Errors that can be ignored
                UserRequestAttemptError::BrokenConnectionError(_)
                | UserRequestAttemptError::UnableToAllocStreamId => true,

                // Handle DbErrors
                UserRequestAttemptError::DbError(db_error, _) => {
                    // Do not remove this lint!
                    // It's there for a reason - we don't want new variants
                    // automatically fall under `_` pattern when they are introduced.
                    #[deny(clippy::wildcard_enum_match_arm)]
    ...
    QueryFut: Future<Output = Result<ResT, UserRequestError>>,
{
    ...
    let mut last_error: UserRequestError;
    loop {
        futures::select! {
            _ = &mut sleep => {
