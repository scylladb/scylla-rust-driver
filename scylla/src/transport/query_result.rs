...
impl QueryResult {
    ...
    pub fn rows_deserializer(
        &self,
    ) -> Result<Option<RowsDeserializer<'_, Borrowing<'_>>>, RowsParseError> {
        self.raw_rows
            .as_ref()
            .map(|raw_rows| {
    ...
    pub fn rows_deserializer_owned(
        &self,
    ) -> Result<Option<RowsDeserializer<'static, Owning>>, RowsParseError> {
        self.raw_rows
            .as_ref()
            .map(|raw_rows| {
...
/// ```
#[derive(Debug)]
pub struct RowsDeserializer<'frame, Kind: RowsDeserializerKind> {
    raw_rows_with_metadata: DeserializedMetadataAndRawRows<'frame, Kind::RawRows>,
    _kind: Kind,
}

pub type RowsDeserializerOwning = RowsDeserializer<'static, Owning>;
pub type RowsDeserializerBorrowing<'a> = RowsDeserializer<'a, Borrowing<'a>>;
...
///
/// Its whole purpose is to restrict the type parameter of `RowsDeserializer`
/// to the two valid variants.
///
/// ### Why is a trait used, and not an enum, as it's done for `ResultMetadataHolder`?
///
/// The problem arises with the deserializing methods, e.g. `rows`.
/// - in case of `RowsDeserializer<Owned>`, the struct itself
...
/// - in case of `RowsDeserializer<Borrowed>`, the struct borrows the frame
///   with some lifetime 'frame. Therefore, the reference to `self` should only have
///   the `'metadata` lifetime, as the frame is owned independently of Self's lifetime.
///
/// This discrepancy is not expressible by enums. Therefore, an entirely separate set
/// of deserializing methods must be defined for both cases, and thus both cases must
/// be separate types - and this is guaranteed by having a different type parameter
/// (because they are distinct instantiations of a generic type).
pub trait RowsDeserializerKind: sealed::Sealed + Debug {
    type RawRows: RawRowsKind;
}

#[derive(Debug)]
pub struct Owning;
impl sealed::Sealed for Owning {}
impl RowsDeserializerKind for Owning {
    type RawRows = RawRowsOwned;
}

#[derive(Debug)]
pub struct Borrowing<'a>(PhantomData<&'a ()>);
impl<'a> sealed::Sealed for Borrowing<'a> {}
impl<'a> RowsDeserializerKind for Borrowing<'a> {
    type RawRows = RawRowsBorrowed<'a>;
}

impl<'frame, Kind: RowsDeserializerKind> RowsDeserializer<'frame, Kind> {
...
impl<'frame> RowsDeserializer<'frame, Borrowing<'frame>> {
...
impl RowsDeserializer<'static, Owning> {
    /// Returns the received rows when present.
    ///
    /// Returns an error if the rows in the response are of incorrect type.
    #[inline]
