 ...
 *   `Connection::execute_iter()` API, which handle separately by treating
 *   all non-Rows results as erroneous).
...
impl QueryPager {
    ...
    async fn next(&mut self) -> Option<Result<(ColumnIterator<'_, '_>, bool), NextRowError>> {
        ...
        // If we executed a USE <keyspace> statement or a DDL statement, there is no current page.
