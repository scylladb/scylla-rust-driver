...
1. The tablet’s Raft leader, for a leader-requiring request against a
   strongly-consistent table — see [Tablet awareness](https://rust-driver.docs.scylladb.com/stable/load-balancing/tablets.md). This
...
Replicas in the same priority groups are shuffled<sup>[1](#id2)</sup>. Non-replicas are randomly
...
Leader-aware routing (see [Tablet awareness](https://rust-driver.docs.scylladb.com/stable/load-balancing/tablets.md)) is a separate
...
---
* <a id='id2'>**[1]**</a> There is an optimisation implemented for LWT requests that routes them to the replicas in the ring order (as it prevents contention due to Paxos conflicts), so replicas in that case are not shuffled in groups at all. The optimisation is applied in two cases:
