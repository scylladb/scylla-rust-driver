...
A `BoundStatement` is a [prepared statement](https://rust-driver.docs.scylladb.com/stable/statements/prepared.md) with its values already
...
- **Serializing the values only once.** `PreparedStatement::calculate_token`
  ...
  [token calculation]() and execution reuse them - one
...
described for [prepared statements](https://rust-driver.docs.scylladb.com/stable/statements/prepared.md).
