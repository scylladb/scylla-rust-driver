...
Additionally, *advanced shard awareness* (the driver choosing a source port to target a specific
shard) is disabled by default because the proxy infrastructure does not preserve it.
**Note:** *basic shard awareness* still works, meaning that driver routes requests optimally - to
the correct shards.
