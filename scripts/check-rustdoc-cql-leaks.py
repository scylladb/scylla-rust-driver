...
def to_int_id(value: Any) -> int:
    """Convert a rustdoc JSON ID to int.

    Raises ValueError/TypeError if the value is not a numeric int or string,
    so that rustdoc format changes are caught loudly instead of causing
    ...
    if isinstance(value, str):
        if value.isdigit():
            return int(value)
        raise ValueError(f"Expected numeric ID string, got {value!r}")
    raise TypeError(
        f"Expected int or numeric string ID, got {type(value).__name__}: {value!r}"
    )
...
def find_path_refs(obj: Any, target_ids: set[int]) -> set[int]:
    ...
    if isinstance(obj, dict):
        if "id" in obj:
            int_id = to_int_id(obj["id"])
            if int_id in target_ids:
                found.add(int_id)
        for v in obj.values():
...
def main() -> None:
    ...
    for crate_id, crate_info in data["external_crates"].items():
        if crate_info["name"] == "scylla_cql":
            scylla_cql_id = to_int_id(crate_id)
            break
    ...
    for path_id, path_info in data["paths"].items():
        if to_int_id(path_info["crate_id"]) == scylla_cql_id:
            scylla_cql_paths[to_int_id(path_id)] = path_info
